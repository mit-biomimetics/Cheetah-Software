...
private:
    void kinematics(const float side_sign,
                    const float q[3], const float dq[3], float * p, float * v, float J[][3]);
    void impedanceControl(const float side_sign      ,
                          const float q[3], const float dq[3],
                          const float position_des[3], const float velocity_des[3],
                          const float kp[3] 		  , const float kd[3],
                          const float force_bias[3]  , const float torque_bias[3],
