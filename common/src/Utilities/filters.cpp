...
template<typename T>
moving_average_filter<T>::moving_average_filter(int num_data): num_data_(num_data),
    idx_(0),
    sum_(0.0){
...
template<typename T>
butterworth_filter<T>::butterworth_filter(int num_sample, T dt, T cutoff_frequency) {
...
template<typename T>
AverageFilter<T>::~AverageFilter(){ est_value_ = 0; }

template<typename T>
void AverageFilter<T>::clear(){ est_value_ = 0.; }
...
template<typename T>
T AverageFilter<T>::output(){ return est_value_; }
