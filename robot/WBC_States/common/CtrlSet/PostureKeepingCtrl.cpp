...
template <typename T>
PostureKeepingCtrl<T>::PostureKeepingCtrl(const FloatingBaseModel<T>* robot):Controller<T>(robot),
...
bool PostureKeepingCtrl<T>::EndOfPhase(){
  if(_sp->_mode == 11){ return true; }
  return false;
}

template <typename T>
void PostureKeepingCtrl<T>::CtrlInitialization(const std::string & category_name){
  if(_param_handler){
