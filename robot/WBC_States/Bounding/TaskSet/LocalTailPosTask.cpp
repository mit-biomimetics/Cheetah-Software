...
template <typename T>
bool LocalTailPosTask<T>::_UpdateCommand(void* pos_des,
    const DVec<T> & vel_des,
    const DVec<T> & acc_des){
  Vec3<T>* pos_cmd = (Vec3<T>*)pos_des;
  Vec3<T> link_pos, link_vel, local_pos, local_vel;
  link_pos = 
    0.5*_robot_sys->_pGC[linkID::HR_abd] + 0.5*_robot_sys->_pGC[linkID::HL_abd];
  link_vel = 
    0.5*_robot_sys->_vGC[linkID::HR_abd] + 0.5*_robot_sys->_vGC[linkID::HL_abd];

  local_pos = 0.5*_robot_sys->_pGC[linkID::HR] + 0.5*_robot_sys->_pGC[linkID::HL];
  local_vel = 0.5*_robot_sys->_vGC[linkID::HR] + 0.5*_robot_sys->_vGC[linkID::HL];

  // X, Y, Z
  for(size_t i(0); i<TK::dim_task_; ++i){
    TK::pos_err_[i] = _Kp_kin[i] * ((*pos_cmd)[i] - (link_pos[i] - local_pos[i]));
    TK::vel_des_[i] = vel_des[i];
    TK::acc_des_[i] = acc_des[i];

    TK::op_cmd_[i] = 
      _Kp[i] * ((*pos_cmd)[i] - (link_pos[i] - local_pos[i]))
      + _Kd[i] * (link_vel[i] - local_vel[i])
      + TK::acc_des_[i];
  }
...
bool LocalTailPosTask<T>::_UpdateTaskJacobian(){
  TK::Jt_ = 
    0.5 * _robot_sys->_Jc[linkID::HR_abd]
