...
template <typename T>
bool LocalHeadPosTask<T>::_UpdateCommand(void* pos_des,
    const DVec<T> & vel_des,
    const DVec<T> & acc_des){
  Vec3<T>* pos_cmd = (Vec3<T>*)pos_des;
  Vec3<T> link_pos, link_vel, local_pos, local_vel;
  link_pos = 
    0.5*_robot_sys->_pGC[linkID::FR_abd] + 0.5*_robot_sys->_pGC[linkID::FL_abd];
  link_vel = 
    0.5*_robot_sys->_vGC[linkID::FR_abd] + 0.5*_robot_sys->_vGC[linkID::FL_abd];

  local_pos = 0.5*_robot_sys->_pGC[linkID::FR] + 0.5*_robot_sys->_pGC[linkID::FL];
  local_vel = 0.5*_robot_sys->_vGC[linkID::FR] + 0.5*_robot_sys->_vGC[linkID::FL];
...
bool LocalHeadPosTask<T>::_UpdateTaskJacobian(){
  TK::Jt_ = 
    0.5 * _robot_sys->_Jc[linkID::FR_abd]
