...
WBIC_TwoContactTransCtrl<T>::WBIC_TwoContactTransCtrl(
    WBICTrotTest<T> * test, const FloatingBaseModel<T>* robot, 
...
void WBIC_TwoContactTransCtrl<T>::_task_setup(){
  ...
  if(_transit_dir<0){
    ...
    _trot_test->_des_jpos = alpha * _trot_test->_des_jpos + (1.-alpha) * _trot_test->_jpos_des_pre;
  }
...
template <typename T>
void WBIC_TwoContactTransCtrl<T>::_SetContact(const size_t & cp_idx, const T & upper_lim){
...
template <typename T>
void WBIC_TwoContactTransCtrl<T>::CtrlInitialization(const std::string & category_name){
...
template <typename T>
void WBIC_TwoContactTransCtrl<T>::SetTestParameter(const std::string & test_file){
